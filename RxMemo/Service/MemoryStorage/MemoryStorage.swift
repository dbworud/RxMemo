...
class MemoryStorage: MemoStorageType {
    ...
    private lazy var store = BehaviorSubject<[Memo]>(value: list) // 외부에서 접근할 필요 없음
    
    @discardableResult
    func create(content: String) -> Observable<Memo> {
        // 새로운 인스턴스 생성
        let memo = Memo(content: content)
        list.insert(memo, at: 0) // 맨 앞에
        
        store.onNext(list) //subject에서 새로운 next event 방출, 이렇게 해야 나중에 tableView와 Binding할 때 정상적으로 변경점 업데이트
    ...
    func update(memo: Memo, content: String) -> Observable<Memo> {
        ...
        if let index = list.firstIndex(where: { $0 == memo}) {
            list.remove(at: index) // 원래 index에 있던 것 제거 후
            list.insert(updated, at : index) // update된 것 insert
        }
